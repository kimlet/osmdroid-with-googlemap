...
public class ColorLinearRing {
    ...
    public List<List<PointL>> startEndPoints = new ArrayList<>();
     ...
     * Dedicated to `Path`
     */
    public ColorLinearRing(final Path pPath) {
        mPath = pPath;
     ...
     * @since 6.0.0
     */
    public ColorLinearRing(final LineBuilder pLineBuilder) {
        mPath = null;
    ...
    void buildLinePortion(final Projection pProjection,
                          final boolean pStorePoints, ColorLineDrawer mLineDrawer) {
        final int size = mOriginalPoints.size();
        if (size < 2) { // nothing to paint
            return;
        }
        ...
        mLineDrawer.setColorList(colorList);
        mLineDrawer.setPathList(pathList);
        mLineDrawer.setStartEndPoints(startEndPoints);
        mLineDrawer.flush();
...
//        mSegmentClipper.end();
    ...
    private void computeProjectedAndDistances(final Projection pProjection) {
        ...
        for (final GeoPoint currentGeo : mOriginalPoints) {
            maxAltitude = currentGeo.getAltitude() > maxAltitude ? currentGeo.getAltitude() : maxAltitude;
//            minAltitude = currentGeo.getAltitude() < minAltitude ? currentGeo.getAltitude() : minAltitude;
    ...
    private void clipAndStore(final Projection pProjection, final PointL pOffset,
        ...
        double currentSpeed = 0, currentAltitude = 0,
                minAltitude = Double.MAX_VALUE, avgAltitude = 0,
                maxSpeed = Double.MIN_VALUE, minSpeed = Double.MAX_VALUE,
                percentColor = 0;
        int speedColors[] = new int[]{Color.parseColor("#06c840"), Color.parseColor("#2cd036")
                , Color.parseColor("#75df24"), Color.parseColor("#99e71b")
                , Color.parseColor("#8bed14")
                , Color.parseColor("#eef906"),
                Color.parseColor("#fee508"),
                Color.parseColor("#febb14"), Color.parseColor("#ffa819"),
                Color.parseColor("#ff8423"), Color.parseColor("#ff6d29")
        };

        List<PointL> startEnd = new ArrayList<>(2);
        ...
        for (int i = 0; i < mProjectedPoints.length; i += 2) {
            ...
            ///////
            currentAltitude = mOriginalPoints.get(index).getAltitude();
            percentColor = currentAltitude / maxAltitude;
            int pos = (int) Math.abs(percentColor * 10);
            pos = pos >= speedColors.length ? speedColors.length - 1 : pos;
            int color = speedColors[pos];


            if (startEnd.size() == 0)// Add start pint
                startEnd.add(new PointL(x, y));


            if (startEnd.size() == 0)// Add start pint
                startEnd.add(new PointL(lastX, lastY));
